...
class RedisMockStringTest(TestCase):
    ...
    def test_get(self):
        ...
        self.assertEquals(val, str_to_bytes("スパム"))

    def test_getset(self):
        val = self.mock.getset('test-key', "new-value")
        self.assertTrue(isinstance(val, bytes))
        self.assertEquals(val, str_to_bytes("スパム"))
        self.assertEquals(self.mock._cache[b'test-key'], str_to_bytes("new-value"))

    def test_incr(self):
        val = self.mock.incr('int-val')
        self.assertTrue(isinstance(val, bytes))
        self.assertEquals(val, str_to_bytes("12"))
        self.assertEquals(self.mock._cache[b'int-val'], str_to_bytes("12"))

    def test_incr_amount(self):
        val = self.mock.incr('int-val', amount=5)
        self.assertTrue(isinstance(val, bytes))
        self.assertEquals(val, str_to_bytes("16"))
        self.assertEquals(self.mock._cache[b'int-val'], str_to_bytes("16"))

    def test_new_incr(self):
        val = self.mock.incr('new-int-val')
        self.assertTrue(isinstance(val, bytes))
        self.assertEquals(val, str_to_bytes("1"))
        self.assertEquals(self.mock._cache[b'new-int-val'], str_to_bytes("1"))

    def test_new_incr_amount(self):
        val = self.mock.incr('new-int-val', amount=4)
        self.assertTrue(isinstance(val, bytes))
        self.assertEquals(val, str_to_bytes("4"))
        self.assertEquals(self.mock._cache[b'new-int-val'], str_to_bytes("4"))

    def test_set(self):
        self.assertTrue(self.mock.set('test-key', "testvalue"))
        self.assertEquals(self.mock._cache[b'test-key'], str_to_bytes("testvalue"))

        self.assertTrue(self.mock.set('new-key', "some-new-testvalue"))
        self.assertEquals(self.mock._cache[b'new-key'], str_to_bytes("some-new-testvalue"))

    def test_setnx(self):
        self.assertFalse(self.mock.setnx('test-key', "testvalue"))
        self.assertEquals(self.mock._cache[b'test-key'], str_to_bytes("スパム"))
    ...
    def test_set_unicode_value(self):
        ...
        self.assertEquals(self.mock._cache[b'test-key'], str_to_bytes("ほげ"))

        self.assertTrue(self.mock.set('new-key', "ほげほげ"))
        self.assertEquals(self.mock._cache[b'new-key'], str_to_bytes("ほげほげ"))
...
class RedisMockListTest(TestCase):
    ...
    def test_set_list(self):
        ...
        self.assertTrue(isinstance(val, bytes))
        self.assertEquals(val, str_to_bytes(str([1, 2, 3])))
    ...
    def test_lpush(self):
        ...
        self.assertEquals(self.mock.lrange('test-int-list', 0, -1), list(map(str_to_bytes, ['10', '1','2','3','4','5','6','7','8'])))

    def test_lpush_unicode_value(self):
        self.assertEquals(self.mock.lpush('test-int-list', "ほげ"), 9)
        self.assertEquals(self.mock._cache[str_to_bytes('test-int-list')], list(map(str_to_bytes, ["ほげ", '1','2','3','4','5','6','7','8'])))

    def test_lpush_not_exists(self):
        self.assertEquals(self.mock.lpush('test-not-exists', 10), 1)
        self.assertEquals(self.mock.lrange('test-not-exists', 0, -1), list(map(str_to_bytes, ['10'])))
        self.assertEquals(self.mock.lpush('test-not-exists', 11), 2)
        self.assertEquals(self.mock.lrange('test-not-exists', 0, -1), list(map(str_to_bytes, ['11', '10'])))
    ...
    def test_rpush(self):
        ...
        self.assertEquals(self.mock.lrange('test-int-list', 0, -1), list(map(str_to_bytes, ['1','2','3','4','5','6','7','8', '10'])))

    def test_rpush_unicode_value(self):
        self.assertEquals(self.mock.rpush('test-int-list', "ほげ"), 9)
        self.assertEquals(self.mock._cache[str_to_bytes('test-int-list')], list(map(str_to_bytes, ['1','2','3','4','5','6','7','8', "ほげ"])))

    def test_lrange_all(self):
        self.assertEquals(self.mock.lrange('test-int-list', 0, -1), list(map(str_to_bytes, ['1','2','3','4','5','6','7','8'])))

    def test_lrange_index_zero_start(self):
        self.assertEquals(self.mock.lrange('test-int-list', 0, 2), list(map(str_to_bytes, ['1','2','3'])))

    def test_lrange_index_nonzero_start(self):
        self.assertEquals(self.mock.lrange('test-int-list', 2, 5), list(map(str_to_bytes, ['3','4','5', '6'])))

    def test_lrange_neg_index(self):
        self.assertEquals(self.mock.lrange('test-int-list', 0, -2), list(map(str_to_bytes, ['1','2','3','4','5','6','7'])))
        self.assertEquals(self.mock.lrange('test-int-list', 2, -4), list(map(str_to_bytes, ['3','4','5'])))
    ...
    def test_ltrim_neg_index(self):
        ...
        self.assertEquals(self.mock.lrange('test-int-list', 0, -1), list(map(str_to_bytes, ['1','2','3','4','5','6','7'])))

    def test_ltrim_neg_index2(self):
        self.assertEquals(self.mock.ltrim('test-int-list', 2, -4), True)
        self.assertEquals(self.mock.lrange('test-int-list', 0, -1), list(map(str_to_bytes, ['3','4','5'])))
    ...
    def test_ltrim_normal(self):
        ...
        self.assertEquals(self.mock.lrange('test-int-list', 0, -1), list(map(str_to_bytes, ['1', '2', '3', '4']))) # ['1', '2', '3'] ではない

    def test_ltrim_normal2(self):
        self.assertEquals(self.mock.ltrim('test-int-list', 2, 5), True)
        self.assertEquals(self.mock.lrange('test-int-list', 0, -1), list(map(str_to_bytes, ['3', '4', '5', '6'])))

    def test_lrem_normal(self):
        self.assertEquals(self.mock.lrem('test-dup-list', 4, 1), 1)
        self.assertEquals(self.mock.lrange('test-dup-list', 0, -1), list(map(str_to_bytes, ['1','3','4','7','4','7','8'])))

    def test_lrem_multi(self):
        self.assertEquals(self.mock.lrem('test-dup-list', 4, 2), 2)
        self.assertEquals(self.mock.lrange('test-dup-list', 0, -1), list(map(str_to_bytes, ['1','3','7','4','7','8'])))

    def test_lrem_over(self):
        self.assertEquals(self.mock.lrem('test-dup-list', 4, 4), 3)
        self.assertEquals(self.mock.lrange('test-dup-list', 0, -1), list(map(str_to_bytes, ['1','3','7','7','8'])))

    def test_lrem_neg_single(self):
        self.assertEquals(self.mock.lrem('test-dup-list', 4, -1), 1)
        self.assertEquals(self.mock.lrange('test-dup-list', 0, -1), list(map(str_to_bytes, ['1','4','3','4','7','7','8'])))

    def test_lrem_neg_multi(self):
        self.assertEquals(self.mock.lrem('test-dup-list', 4, -2), 2)
        self.assertEquals(self.mock.lrange('test-dup-list', 0, -1), list(map(str_to_bytes, ['1','4','3','7','7','8'])))
...
class RedisMockSetTest(TestCase):
    ...
    def test_sadd_unicode_value(self):
        ...
        self.assertEquals(self.mock._cache[b'test-string-set'], set(map(str_to_bytes, [
            "スパム",
            "エッグ",
            "ほげ",
        ])))
...
class RedisMockHashTest(TestCase):
    ...
    def test_hgetall(self):
        ...
        self.assertEquals(val, self.mock._cache[b'test-hash'])
    ...
    def test_hset_unicode(self):
        ...
        self.assertEquals(val, 1)
        self.assertEquals(self.mock._cache[b'test-hash'].get(str_to_bytes("ほげ")), str_to_bytes("ホゲ"))
...
class RedisPipelineTest(TestCase):
    ...
    def test_simple_pipeline(self):
        ...
        value = pipe.execute()
        self.assertEquals(value, [str_to_bytes("スパム"), str_to_bytes("エッグ"), None])
...
class RedisVsMock(TestCase):
    ...
    def test_string(self):
        ...
        set('test-key', 1)
        self.assertEquals(*get('test-key'))
        set('test-key', 1.23)
        self.assertEquals(*get('test-key'))
        set('test-key', "スパム")
        self.assertEquals(*get('test-key'))
        set('test-key2', "エッグ")
        self.assertEquals(*get('test-key2'))
    ...
    def test_list(self):
        ...
        self.assertEquals(*lrange('test-list'))
    ...
    def test_hash(self):
        ...
        self.assertEquals(*hget('test-hash', 'hashkey1'))
        self.assertEquals(*hget('test-hash', 'hashkey2'))
    ...
    def test_set(self):
        ...
        self.assertEquals(*smembers('test-string-set'))
        self.assertEquals(*smembers('test-int-set'))
